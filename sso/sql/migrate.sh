...
for f in /var/www/api/sso/sql/diff/[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]-*.sql; do
    if [ -f "$f" ]; then
        diff="$(echo $f | grep -oP '(?<=diff/).*(?=$)')" # diff name.

        echo -n "Checking if $diff has already been applied... "
        res="$(mysql --vertical -e "SELECT * FROM sso.migrations WHERE name = '$diff';" | grep sql)"
        if [ "$res" != "" ]; then
            echo "applied."
        else
            ...
            if [ $? -eq 0 ]; then
                ...
                mysql -e "INSERT INTO sso.migrations (name) VALUES ('$diff');"
                if [ $? -eq 0 ]; then
                    echo "Done."
                else
                    echo "Error."
